...
class Iso6392LanguageCatalog
{
    ...
    /** @var array<string, string> canonical bibliographic code => primary English name */
    protected array $englishByBibliographic = [];

    /** @var array<string, string> canonical bibliographic code => ISO 639-1 code */
    protected array $iso6391ByBibliographic = [];
    ...
    public function englishNameForBibliographic(string $code): ?string
    {
        ...
        return $this->englishByBibliographic[$code] ?? null;
    }

    public function iso6391ForBibliographic(string $code): ?string
    {
        $code = strtolower(trim($code));
        if ($code === '') {
            return null;
        ...
        return $this->iso6391ByBibliographic[$code] ?? null;
    }
    ...
    protected function loadIso6392File(string $path): void
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($iso6391 !== '') {
                ...
                $this->iso6391ByBibliographic[$canonical] = $iso6391;
            }
            if ($english !== '') {
    ...
    protected function registerEnglishNames(string $english, string $bibliographic): void
    {
        foreach (explode(';', $english) as $part) {
            ...
            if (!isset($this->englishByBibliographic[$bibliographic])) {
                $this->englishByBibliographic[$bibliographic] = $part;
            }
        }
    }
